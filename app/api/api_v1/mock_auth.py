...
@mock_auth_route.post(
    "/mock_sing_up/",
    response_model=AccessRefreshToken,
    summary="Mock Social Sign Up User",
)
async def mock_sing_up(user_auth_data: UserAuthData = Depends()) -> AccessRefreshToken:
    ...
    user = await user_service.create_user(user_auth_data)
...
@mock_auth_route.post(
    "/mock_sing_in/",
    response_model=AccessRefreshToken,
    summary="Mock Social Sign In User",
)
